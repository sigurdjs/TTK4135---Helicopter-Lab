...
options = optimoptions('fmincon','Algorithm', ...
    'sqp','Display','iter','UseParallel',true);
tic
z = fmincon(obj_fun,z0,[],[],Aeq,beq,vlb,vub, ...
    @nonlinear_constraint,options);
% [z,lambda] = quadprog(Q,c,[],[],Aeq,beq,vlb,vub);
t1=toc;

%% Find K matrix with LQR
Q_lq = diag([10 16/(pi^2) 16/(pi^2) ...
        4/(pi^2) 16/(pi^2) 4/(pi^2)]);
R_lq = diag([1/pitch_lim_pos^2 1/pitch_lim_pos^2]);
